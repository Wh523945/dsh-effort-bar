...
一根**常驻在输入框里、极简风格**的推理强度滑条。

只做滑条 —— 无特效，也不替换官方的任何控件。
...
| **拖动预览** | 按住拖动时滑条上方浮出磨砂小气泡，显示当前停留的档位名。 |
| **松手提交** | 指针松开 / 键盘操作结束 / 控件失焦时，通过官方 `ModelDirectory` 提交该档位，与 `/model` 弹窗、composer 模型座位共享同一份状态。 |
